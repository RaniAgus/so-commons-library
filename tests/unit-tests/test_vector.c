...
static t_person person_create(char* name, int age)
{
    return (t_person)
    {
...
context (test_vector)
{
    ...
    inline void assert_person_at_position(t_vector* vector, size_t pos, char* name, int age)
    {
        assert_person(vector_get(vector, pos), name, age);
    }
    ...
    describe ("Vector")
    {
        ...
        before
        {
            vector_create(&vec, sizeof(t_person), (void*) person_destroy, 0);
        } end
        ...
        describe("Insertions")
        {
            it ("should insert a value at back")
            {
                ...
                vector_push_last(&vec, &temp);
                assert_person(vector_last(&vec), "Matias", 24);

                temp = person_create("Gaston", 25);
                vector_push_last(&vec, &temp);
                assert_person(vector_last(&vec), "Gaston", 25);
            ...
            it("should add all elements from array at end")
            {
                ...
                vector_add_from_array(&vec, vector_size(&vec), &others[0], &others[2]);
        ...
        describe ("Higher order functions")
        {
            ...
            it("should filter a vector with the values that satisfies a condition")
            {
                ...
                t_vector young_people;
                vector_create(&young_people, sizeof(t_person), NULL, 0);
                void _addYoungPeople(t_person* person)
                {
                    if (person->age <= 24)
                        vector_push_last(&young_people, person);
                }
                vector_iterate(&vec, (void*) _addYoungPeople);
            ...
            it("should map a vector with the function result")
            {
                ...
                t_vector names;
                vector_create(&names, sizeof(char*), NULL, 0);
                void _getNames(t_person* person)
                {
                    vector_push_last(&names, &person->name);
                }

                vector_iterate(&vec, (void*) _getNames);
                for (size_t i = 0; i < 5; ++i)
                {
                    should_string(*(char**) vector_get(&names, i)) be equal to(names_array[i]);
                }
        ...
        describe ("Swap")
        {
            ...
            {
                t_vector other;
                vector_create(&other, sizeof(t_person), (void*) person_destroy, 0);

                temp = person_create("Lucas", 31);
                vector_push_last(&other, &temp);
